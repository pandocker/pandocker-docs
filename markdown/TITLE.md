...
また、pandockerを継承した派生Dockerイメージを作る際にpandoc-miscパッケージを上書きするだけで
フレームワークを差し替えることもできるようになります。
...
[@sec:updates-for-docx-output]でいろいろ書いてある段落スタイルの設定ですが、フィルタを使わずとも
指定することはできます。`div`節に`custom-style="XX"`というオプションを与えると、"XX"が
テンプレート内に段落スタイルとして定義済であれば適用され、未定義なら"標準"スタイルを適用します。

Pandoc本家のマニュアルに詳細が書かれています[^custom-styles-in-docx]ので参照してください。
現在最新版(2.5)のマニュアルを翻訳するプロジェクトが進められています。

[^custom-styles-in-docx]: <https://pandoc.org/MANUAL.html#custom-styles-in-docx>

この記法を駆使すれば柔軟にDOCXを編集できそうというのはわかるのですが、番号なし見出しを
多用する場合などはやってられないと思い、各種フィルタを追加で作る考えに至りました。

### 箇条書きスタイルを適用させる例 {-}

箇条書きの記法を使えばDOCXでも箇条書きとして出力されますが、かなりシンプルなスタイルになります。
この見た目が筆者的に好かないので、段落スタイルとして箇条書きのスタイル"Bullet List 1"を
適用したいと思います。

1. テンプレートに"Bullet List 1"を用意しておきます。必ずテンプレートファイル内のどこかに
スタイル適用済の文章を入れておきます。Pandocは**実際に使われている**段落とテーブルのスタイルの情報を
入手して変換に使用します。
2. 原稿mdファイルに以下のようなdiv節を追加します。
   divの内部は複数行になっても問題ありません。ただし行頭のマークは一つのdiv節で一つだけつきます。
   ```markdown
...
これも項目が多いときなどはやってられないので、フィルタを作って箇条書き記法から変換させたいのですが、
木構造の深さ情報読み出し部分がうまく行っていません。
